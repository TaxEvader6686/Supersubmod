...
function GovernmentEmpire:new(gc, absorb, dark_empire_available, id)
    ...
    self.zce_table = {
        ["CORPORATE_SECTOR"] = {
            is_integrated = false,
            destruction_unlocks = {"Zsinj_Ship_Market", "SliroUpgrade"},
            destruction_unlock_descs = {TEXT_GOVERNMENT_EMPIRE_INTEGRATION_REWARD_CSA},
        },
        ["HUTT_CARTELS"] = {
            is_integrated = false,
            destruction_unlocks = {"Barabbula_Frigate", "Karagga_Destroyer", "Vontor_Destroyer", "Voracious_Carrier", "TeubboUpgrade"},
            destruction_unlock_descs = {TEXT_GOVERNMENT_EMPIRE_INTEGRATION_REWARD_HUTT},
        },
	}

    self.zce_pending_integration_queue = {}
...
function GovernmentEmpire:update()
    ...
    for _, integrated_faction in pairs(self.zce_pending_integration_queue) do
        local player_integrated_faction = Find_Player(integrated_faction)
		local zsinj_player_faction = Find_Player("ZSINJ_EMPIRE")
	    if player_integrated_faction == Find_Player("CORPORATE_SECTOR") then
            GlobalValue.Set("ZSINJ_SHIP_MARKET_UNLOCK", "TRUE")
        end        
        Faction_Total_Replace(player_integrated_faction, zsinj_player_faction, 1)
        
        if self.zce_table[integrated_faction].destruction_unlocks then  
            UnitUtil.SetLockList(integrated_faction, self.zce_table[integrated_faction].destruction_unlocks, false)
            UnitUtil.SetLockList("ZSINJ_EMPIRE", self.zce_table[integrated_faction].destruction_unlocks)
            for _, unit in pairs(self.zce_table[integrated_faction].destruction_unlocks) do
                table.insert(self.imperial_table["ZSINJ_EMPIRE"].destruction_unlocks, unit)
            end
            for _, desc in pairs(self.zce_table[integrated_faction].destruction_unlock_descs) do
                table.insert(self.imperial_table["ZSINJ_EMPIRE"].destruction_unlock_descs, desc)
            end
            self.zce_table[integrated_faction].destruction_unlocks = {}
            self.zce_table[integrated_faction].destruction_unlock_descs = {}
        end
	end
...
function GovernmentEmpire:check_for_integration(old_owner_name)
    if not (self.imperial_table[old_owner_name] or self.zce_table[old_owner_name]) then
        return
    end
    if (self.imperial_table[old_owner_name] and self.imperial_table[old_owner_name].is_integrated == true) or (self.zce_table[old_owner_name] and self.zce_table[old_owner_name].is_integrated == true) then
        return
    end
    if (self.imperial_table[old_owner_name] and self.imperial_table[old_owner_name].pending_integration == true) or (self.zce_table[old_owner_name] and self.zce_table[old_owner_name].pending_integration == true) then
        return
    end
	...
	elseif self.zce_table[old_owner_name] then
        if self.zce_table[old_owner_name].is_integrated == false and self.imperial_table["ZSINJ_EMPIRE"].controls_planets == true and self.imperial_table["ZSINJ_EMPIRE"].is_integrated == false then
			if self.LegitimacyAbsorb > 0 or table.getn(self.zce_table[old_owner_name].destruction_unlocks) > 0 then
				if GlobalValue.Get("GOV_EMP_DISABLE_MULTIMEDIA_HOLO") == 1 then
					StoryUtil.Multimedia("TEXT_GOVERNMENT_LEGITIMACY_ABSORB_SPEECH_"..tostring(old_owner_name), 15, nil, nil, 0, nil, {r = 255, g = 255, b = 100}) 
					GlobalValue.Set("GOV_EMP_DISABLE_MULTIMEDIA_HOLO", 0)
				else
			...
			self.zce_table[old_owner_name].is_integrated = true
			table.insert(self.zce_pending_integration_queue, old_owner_name)
        end
    end
end
...
function GovernmentEmpire:dark_empire_choice_made(option,ai_faction_name)
    ...
    elseif option == "DARK_EMPIRE_CHOICE_REFUSE" then
        if self.HighestLegitimacy == "ZSINJ_EMPIRE" and self.zce_table["HUTT_CARTELS"].is_integrated == true then
